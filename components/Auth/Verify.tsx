...
const Verify = () => {
  ...
  if (isPageLoading) {
    return (
      <div className="h-screen flex justify-center items-center">
        <Loader className="w-20 h-20 animate-ping" />
      </div>
    )
  }
  ...
  return (
    <div className="h-screen flex items-center flex-col justify-center">
      <MailCheck className="w-20 h-20 sm:w-32 sm:h-32 text-red-500 mb-12" />
      <h1 className="text-2xl sm:text-3xl font-bold mb-3">
        Vérification par mail
      </h1>
      <p className="mb-6 text-sm sm:text-base text-gray-600 font-medium">
        Nous avons envoyé un code à {user?.email}
      </p>
      ...
      <LoadingButton
        ...
        isLoading={isLoading}
        size={"lg"}
        className="mt-6 w-52 text-white"
      >
        Vérifier votre code
      </LoadingButton>
    </div>
